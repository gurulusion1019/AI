...
# ── Accenture branding ────────────────────────────────────────────────────
# Change these in ONE place to restyle every generated report.
ACCENTURE_PURPLE = "#A100FF"      # primary brand colour
ACCENTURE_DARK   = "#460073"      # deep purple, for secondary text
TITLE_FONT_SIZE  = "28pt"         # report title size (was 20pt)
TITLE_FONT_FAMILY = "Segoe UI Semibold"
TITLE_FONT_COLOR = "#FFFFFF"      # white text on the purple band
CARD_VALUE_FONT_SIZE = 30         # KPI number size (Power BI default is 45)
CARD_LABEL_FONT_SIZE = 11         # KPI caption size
TITLE_HEIGHT     = 60             # title band height in px
...
def _pbir_report_json() -> dict:
    ...
    Kept minimal to satisfy the report/2.0.0 schema — Power BI rejects extra
    root properties (e.g. layoutOptimization) and unknown settings keys
    (e.g. useNewFilterPaneExperience), so only schema-allowed fields are here.
    """
    return {
...
def _pbir_page_json(page_id: str, display_name: str, height: int = None) -> dict:
    """CRITICAL: schema 2.0.0 allows ONLY these 6 properties — no extras.

    `height` grows with the content: with two chart rows plus a slicer strip
    the page needs more than the default 720px, otherwise the lower visuals
    (typically the slicers) sit below the canvas and are invisible.
    """
    return {
        ...
        "height": int(height or PBIR_PAGE_H),
        "width": PBIR_PAGE_W
    }
...
def _pbir_title_visual(title_text: str, x: int, y: int, w: int, h: int) -> dict:
    """
    Report title: WHITE text on an Accenture-purple band.

    The coloured band is a visual-container background, which lives under
    `visualContainerObjects.background` (NOT `objects`) — that is where Power
    BI itself writes it.
    """
    return {
                ...
                "background": [
                    {
                        "properties": {
                            "show": {"expr": {"Literal": {"Value": "true"}}},
                            "color": {"solid": {"color": {
                                "expr": {"Literal": {"Value": f"'{ACCENTURE_PURPLE}'"}}}}},
                            "transparency": {"expr": {"Literal": {"Value": "0D"}}},
                        }
                    }
                ]
...
def _pbir_card_visual(measure_name: str, fact_table: str,
    ...
    Font sizes are set explicitly (Power BI's default is 45pt, which is too
    large for a compact card and clips the category label):
      labels        -> the big number   (CARD_VALUE_FONT_SIZE)
      categoryLabels-> the caption text (CARD_LABEL_FONT_SIZE)
    """
    def _lit(v):
    ...
    return {
        ...
        "visual": {
            ...
            "objects": {
                "labels": [{"properties": {
                    "fontSize": _lit(f"{CARD_VALUE_FONT_SIZE}D")}}],
                "categoryLabels": [{"properties": {
                    "fontSize": _lit(f"{CARD_LABEL_FONT_SIZE}D")}}],
            }
        }
    }
...
def _build_report_default(model: SemanticModel, report_folder: str) -> dict:
    """
    DEFAULT / FALLBACK layout — the proven fixed layout (title + KPI cards +
    bar/line/donut + slicers). Used when the AI Visual Planner is unavailable
    or returns nothing. This is the exact code that produced working dashboards
    before the planner existed, kept intact as a safety net.
    ...
    # Third chart — SMART, cardinality-aware:
    #   A donut is only readable with few slices. So pick the dimension whose
    #   category column has the FEWEST distinct values; use a donut only if that
    #   is <= DONUT_MAX_CATEGORIES, otherwise fall back to a (scrollable) bar.
    #   This is why a 40-customer donut is avoided in favour of, say, a
    #   10-channel donut — chosen dynamically from unique_count, no hardcoding.
    DONUT_MAX_CATEGORIES = 12

    def _cardinality(meta, col_name):
        for c in meta.columns:
            if c.name == col_name:
                return getattr(c, "unique_count", None)
        return None

    if len(dim_tables) > 1 and have_value:
        used = {dim_tables[0].table_name}          # bar already used dim 0
        # rank remaining dims by their category cardinality (ascending)
        ranked = []
        for dt in dim_tables:
            if dt.table_name in used:
                continue
            c = _pick_category(dt)
            if not c:
                continue
            card = _cardinality(dt, c)
            ranked.append((card if card is not None else 9_999, dt, c))
        ranked.sort(key=lambda x: x[0])

        if ranked:
            best_card, dt, cat = ranked[0]
            if best_card <= DONUT_MAX_CATEGORIES:
                vtype, is_donut = "donutChart", True    # few slices → readable donut
            else:
                vtype, is_donut = "clusteredBarChart", False  # too many → bar
            cfg = _emit_chart(vtype, cat, dt.table_name, 22, chart_x, donut=is_donut)
            if cfg:
                visual_containers.append({
                    "config": cfg, "filters": "[]",
                    "height": float(chart_h), "width": float(chart_w),
...
# Role names differ per visual type — taken from real Power BI files.
# (category_role, value_role)
_PBIR_ROLES = {
    "barChart":                     ("Category", "Y"),   # stacked bar
    "clusteredBarChart":            ("Category", "Y"),
    "columnChart":                  ("Category", "Y"),   # stacked column
    ...
    "treemap":                      ("Group", "Values"),  # different roles
    "map":                          ("Category", "Size"), # different value role
}


def _pbir_col_field(table: str, column: str) -> dict:
    """A column reference field."""
    return {"Column": {"Expression": {"SourceRef": {"Entity": table}},
                       "Property": column}}


def _pbir_measure_field(table: str, measure: str) -> dict:
    """A measure reference field."""
    return {"Measure": {"Expression": {"SourceRef": {"Entity": table}},
                        "Property": measure}}


def _pbir_visual_shell(visual_type: str, query_state: dict,
                        x: int, y: int, w: int, h: int, tab: int) -> dict:
    """Common visual.json wrapper shared by every visual type."""
    return {
        "$schema": f"{PBIR_SCHEMA_BASE}/visualContainer/2.0.0/schema.json",
        "name": _pbir_id(),
        "position": {"x": x, "y": y, "z": tab, "height": h, "width": w, "tabOrder": tab},
        "visual": {
            "visualType": visual_type,
            "drillFilterOtherVisuals": True,
...
def _pbir_shapemap_visual(category_col: str, cat_table: str,
                           measure_name: str, fact_table: str,
                           x: int, y: int, w: int, h: int, tab: int) -> dict:
    """Shape map: Category (geographic column) + Value (measure)."""
    qs = {
        "Category": {"projections": [
            {"field": _pbir_col_field(cat_table, category_col),
             "queryRef": f"{cat_table}.{category_col}",
             "nativeQueryRef": category_col, "active": True}]},
        "Value": {"projections": [
            {"field": _pbir_measure_field(fact_table, measure_name),
             "queryRef": f"{fact_table}.{measure_name}", "nativeQueryRef": measure_name}]},
    }
    return _pbir_visual_shell("shapeMap", qs, x, y, w, h, tab)


def _pbir_decomptree_visual(measure_name: str, fact_table: str,
                             explain_by: list,
                             x: int, y: int, w: int, h: int, tab: int) -> dict:
    """
    Decomposition tree: Analyze (one measure) + ExplainBy (columns to drill).
    `explain_by` = [{"table": t, "column": c}, ...]
    """
    qs = {
        "Analyze": {"projections": [
            {"field": _pbir_measure_field(fact_table, measure_name),
        ...
        "ExplainBy": {"projections": [
            {"field": _pbir_col_field(f["table"], f["column"]),
             "queryRef": f"{f['table']}.{f['column']}", "nativeQueryRef": f["column"]}
            for f in explain_by]},
    }
    return _pbir_visual_shell("decompositionTreeVisual", qs, x, y, w, h, tab)


def _pbir_scatter_visual(category_col: str, cat_table: str,
                          x_measure: str, y_measure: str, fact_table: str,
                          x: int, y: int, w: int, h: int, tab: int) -> dict:
    """Scatter chart: Category + X measure + Y measure (roles from template)."""
    qs = {
        "Category": {"projections": [
            {"field": _pbir_col_field(cat_table, category_col),
             "queryRef": f"{cat_table}.{category_col}",
             "nativeQueryRef": category_col, "active": True}]},
        "X": {"projections": [
            {"field": _pbir_measure_field(fact_table, x_measure),
             "queryRef": f"{fact_table}.{x_measure}", "nativeQueryRef": x_measure}]},
        "Y": {"projections": [
            {"field": _pbir_measure_field(fact_table, y_measure),
             "queryRef": f"{fact_table}.{y_measure}", "nativeQueryRef": y_measure}]},
    }
    return _pbir_visual_shell("scatterChart", qs, x, y, w, h, tab)


def _pbir_table_visual(fields: list, fact_table: str,
                       x: int, y: int, w: int, h: int, tab: int) -> dict:
    """
    Table (tableEx): a single 'Values' role holding several projections, which
    may be columns and/or measures.
    `fields` = [{"kind":"column","table":t,"name":c} | {"kind":"measure","name":m}]
    """
    projections = []
    for f in fields:
        if f.get("kind") == "measure":
            projections.append({
                "field": _pbir_measure_field(fact_table, f["name"]),
                "queryRef": f"{fact_table}.{f['name']}",
                "nativeQueryRef": f["name"]})
        else:
            projections.append({
                "field": _pbir_col_field(f["table"], f["name"]),
                "queryRef": f"{f['table']}.{f['name']}",
                "nativeQueryRef": f["name"]})
    return _pbir_visual_shell("tableEx", {"Values": {"projections": projections}},
                              x, y, w, h, tab)


def _pbir_matrix_visual(row_table: str, row_column: str, measures: list,
                        fact_table: str,
                        x: int, y: int, w: int, h: int, tab: int) -> dict:
    """Matrix (pivotTable): Rows (a column) + Values (measures)."""
    qs = {
        "Rows": {"projections": [
            {"field": _pbir_col_field(row_table, row_column),
             "queryRef": f"{row_table}.{row_column}",
             "nativeQueryRef": row_column, "active": True}]},
        "Values": {"projections": [
            {"field": _pbir_measure_field(fact_table, m),
             "queryRef": f"{fact_table}.{m}", "nativeQueryRef": m}
            for m in measures]},
    }
    return _pbir_visual_shell("pivotTable", qs, x, y, w, h, tab)


def _pbir_gauge_visual(measure_name: str, fact_table: str,
                        min_measure: str = None, max_measure: str = None,
                        x: int = 0, y: int = 0, w: int = 400, h: int = 250,
                        tab: int = 0) -> dict:
    """Gauge: Y (the value) plus optional MinValue / MaxValue measures."""
    qs = {"Y": {"projections": [
        {"field": _pbir_measure_field(fact_table, measure_name),
         "queryRef": f"{fact_table}.{measure_name}", "nativeQueryRef": measure_name}]}}
    if min_measure:
        qs["MinValue"] = {"projections": [
            {"field": _pbir_measure_field(fact_table, min_measure),
             "queryRef": f"{fact_table}.{min_measure}", "nativeQueryRef": min_measure}]}
    if max_measure:
        qs["MaxValue"] = {"projections": [
            {"field": _pbir_measure_field(fact_table, max_measure),
             "queryRef": f"{fact_table}.{max_measure}", "nativeQueryRef": max_measure}]}
    return _pbir_visual_shell("gauge", qs, x, y, w, h, tab)


def _pbir_kpi_visual(measure_name: str, fact_table: str,
                      goal_measure: str = None,
                      trend_table: str = None, trend_column: str = None,
                      x: int = 0, y: int = 0, w: int = 400, h: int = 250,
                      tab: int = 0) -> dict:
    """KPI visual: Indicator (value) + optional Goal measure + TrendLine column."""
    qs = {"Indicator": {"projections": [
        {"field": _pbir_measure_field(fact_table, measure_name),
         "queryRef": f"{fact_table}.{measure_name}", "nativeQueryRef": measure_name}]}}
    if goal_measure:
        qs["Goal"] = {"projections": [
            {"field": _pbir_measure_field(fact_table, goal_measure),
             "queryRef": f"{fact_table}.{goal_measure}", "nativeQueryRef": goal_measure}]}
    if trend_table and trend_column:
        qs["TrendLine"] = {"projections": [
            {"field": _pbir_col_field(trend_table, trend_column),
             "queryRef": f"{trend_table}.{trend_column}", "nativeQueryRef": trend_column}]}
    return _pbir_visual_shell("kpi", qs, x, y, w, h, tab)


def _pbir_chart_visual(visual_type: str, category_col: str, cat_table: str,
                        measure_name: str, fact_table: str,
                        x: int, y: int, w: int, h: int, tab: int,
                        series_table: str = None, series_column: str = None) -> dict:
    """
    Category + measure visual (bar, column, line, area, pie, donut, treemap).
    Role names are looked up per visual type from _PBIR_ROLES, because they
    differ — e.g. treemap uses Group/Values instead of Category/Y.
    """
    cat_role, val_role = _PBIR_ROLES.get(visual_type, ("Category", "Y"))
    qs = {
        cat_role: {"projections": [
            {"field": _pbir_col_field(cat_table, category_col),
             "queryRef": f"{cat_table}.{category_col}",
             "nativeQueryRef": category_col, "active": True}]},
        val_role: {"projections": [
            {"field": _pbir_measure_field(fact_table, measure_name),
             "queryRef": f"{fact_table}.{measure_name}",
             "nativeQueryRef": measure_name}]},
    }
    if series_table and series_column:
        # ribbon / 100%-stacked use a Series role to split each category
        qs["Series"] = {"projections": [
            {"field": _pbir_col_field(series_table, series_column),
             "queryRef": f"{series_table}.{series_column}",
             "nativeQueryRef": series_column}]}
    return _pbir_visual_shell(visual_type, qs, x, y, w, h, tab)


def _pbir_slicer_visual(column_name: str, dim_table: str,
                         x: int, y: int, w: int, h: int, tab: int) -> dict:
    """
    Slicer visual on a dimension column (modern PBIR).

    Settings applied (property names taken from real Power BI files, where
    values are wrapped as expr.Literal.Value):
      * Style             -> Dropdown   (data.mode = 'Dropdown')
      * Single select     -> Off        (selection.singleSelect = false)
      * Show "Select all" -> On         (selection.selectAllCheckboxEnabled = true)
    """
    field = {
        "Column": {
            "Expression": {"SourceRef": {"Entity": dim_table}},
            "Property": column_name,
        }
    }
    ...
    return {
        ...
        "visual": {
            ...
            "query": {
                "queryState": {
                    "Values": {"projections": [
                        {"field": field, "queryRef": f"{dim_table}.{column_name}"}]}
                }
            },
            "objects": {
                # Verified against real Power BI slicers:
                #   data.mode = 'Dropdown'  (list style is the default)
                "data": [{"properties": {"mode": _lit("'Dropdown'")}}],
                "selection": [{"properties": {
                    "selectAllCheckboxEnabled": _lit("true"),   # Show "Select all" ON
                    "singleSelect": _lit("false"),              # Single select OFF
                }}],
            },
        },
    }
...
# Plan visual "type" -> PBIR visualType name. Any type here is renderable.
_PLAN_TO_PBIR_TYPE = {
    ...
    "ribbon":         "ribbonChart",
    "stacked100_bar": "hundredPercentStackedBarChart",
    "map":            "map",
}
# Types that occupy a chart slot in the grid (incl. the wide ones)
_CHART_TYPES = set(_PLAN_TO_PBIR_TYPE) | {"scatter", "table", "matrix", "gauge", "kpi", "shapemap", "decomptree"}


def _plan_to_pbir_visuals(plan: list, model: SemanticModel) -> list:
    """
    Turn the neutral visual plan into a list of PBIR visual dicts.

    Layout (top to bottom): title band, SLICER strip, KPI cards, then up to 6
    chart-slot visuals across two rows (3 per row). Table/matrix take a chart
    slot but are rendered wider since they need horizontal room.
    """
    PAGE_MAX_CARDS, PAGE_MAX_CHARTS, PAGE_MAX_SLICERS = 4, 6, 5
    fact_table = model.tables[0].table_name if model.tables else None
    visuals = []

    visuals.append(_pbir_title_visual(model.project_name or "Report", 24, 20, 1232, TITLE_HEIGHT))

    cards   = [v for v in plan if v.get("type") == "card"][:PAGE_MAX_CARDS]
    charts  = [v for v in plan if v.get("type") in _CHART_TYPES][:PAGE_MAX_CHARTS]
    slicers = [v for v in plan if v.get("type") == "slicer"][:PAGE_MAX_SLICERS]

    # ── 1) Slicer strip: straight under the title band ────────────────────
    SLICER_Y, SLICER_H, SLICER_W = 100, 55, 240
    sx = 24
    for k, v in enumerate(slicers):
        visuals.append(_pbir_slicer_visual(v["column"], v["table"],
                                           sx, SLICER_Y, SLICER_W, SLICER_H, 10 + k))
        sx += SLICER_W + 16

    # ── 2) KPI cards: below the slicers, compact so they don't dominate ───
    CARD_Y = (SLICER_Y + SLICER_H + 15) if slicers else 100
    CARD_H = 85                      # was 120 — smaller, less wasted space
    if fact_table and cards:
        n = len(cards); margin = 24
        cw = int((1280 - 2 * margin - (n - 1) * margin) / n)
        for i, v in enumerate(cards):
            cx = margin + i * (cw + margin)
            visuals.append(_pbir_card_visual(v["measure"], fact_table,
                                             cx, CARD_Y, cw, CARD_H, i + 1))

    # ── 3) Charts start below the cards ───────────────────────────────────
    charts_top = CARD_Y + CARD_H + 20
    col_x = [24, 24 + 416, 24 + 832]
    row_y = [charts_top, charts_top + 270]
    placed = 0
    for v in charts:
        if not fact_table:
            break
        if placed // 3 >= len(row_y):
            break                      # grid full (wide visuals take 2 slots)
        vt = v.get("type")
        cx, cy = col_x[placed % 3], row_y[placed // 3]
        w, h = 400, 250
        vis = None

        if vt in _PLAN_TO_PBIR_TYPE:
            # category + measure chart (bar/column/line/area/pie/donut/treemap)
            if vt in ("line", "area"):
                ct, cc = v.get("axis_table"), v.get("axis_column")
            else:
                ct, cc = v.get("category_table"), v.get("category_column")
            if ct and cc:
                vis = _pbir_chart_visual(_PLAN_TO_PBIR_TYPE[vt], cc, ct,
                                         v["measure"], fact_table,
                                         cx, cy, w, h, 20 + placed,
                                         series_table=v.get("series_table"),
                                         series_column=v.get("series_column"))

        elif vt == "gauge":
            vis = _pbir_gauge_visual(v["measure"], fact_table,
                                     v.get("min_measure"), v.get("max_measure"),
                                     cx, cy, w, h, 20 + placed)

        elif vt == "kpi":
            vis = _pbir_kpi_visual(v["measure"], fact_table,
                                   v.get("goal_measure"),
                                   v.get("trend_table"), v.get("trend_column"),
                                   cx, cy, w, h, 20 + placed)

        elif vt == "shapemap":
            ct, cc = v.get("category_table"), v.get("category_column")
            if ct and cc:
                vis = _pbir_shapemap_visual(cc, ct, v["measure"], fact_table,
                                            cx, cy, w, h, 20 + placed)

        elif vt == "decomptree":
            eb = v.get("explain_by") or []
            if eb:
                w = 816 if (placed % 3) < 2 else 400   # tree needs width
                vis = _pbir_decomptree_visual(v["measure"], fact_table, eb,
                                              cx, cy, w, h, 20 + placed)

        elif vt == "scatter":
            ct, cc = v.get("category_table"), v.get("category_column")
            xm, ym = v.get("x_measure"), v.get("y_measure")
            if ct and cc and xm and ym:
                vis = _pbir_scatter_visual(cc, ct, xm, ym, fact_table,
                                           cx, cy, w, h, 20 + placed)

        elif vt == "table":
            fields = v.get("fields") or []
            if fields:
                # tables need width: use two grid columns
                w = 816 if (placed % 3) < 2 else 400
                vis = _pbir_table_visual(fields, fact_table, cx, cy, w, h, 20 + placed)

        elif vt == "matrix":
            rt, rc = v.get("row_table"), v.get("row_column")
            ms = v.get("measures") or ([v["measure"]] if v.get("measure") else [])
            if rt and rc and ms:
                w = 816 if (placed % 3) < 2 else 400
                vis = _pbir_matrix_visual(rt, rc, ms, fact_table, cx, cy, w, h, 20 + placed)

        if vis:
            visuals.append(vis)
            # wide visuals consume two grid columns
            placed += 2 if w > 500 else 1

    return visuals


def build_pbir_modern_files(model: SemanticModel, report_folder: str,
                            user_requests: str = "") -> dict:
    """
    MODERN PBIR renderer. Emits the definition/ FOLDER structure that current
    Power BI Desktop (2026+) requires, instead of the legacy flat report.json.

    Layout inside {report_folder}/:
    ...
    Uses the AI Visual Planner (planner-first) with the same fallback as the
    legacy path — if the planner returns nothing, a minimal default plan is used.
    """
    # Get the plan (planner-first, same as legacy)
    plan = []
    try:
        from .visual_planner import build_visual_plan
        plan = build_visual_plan(model, user_requests or "")
    except Exception as exc:
        logger.warning("Visual planner unavailable (%s) — PBIR default plan", exc)
    if not plan:
        # minimal safe default: cards for first measures + one bar if possible
        measures = [(getattr(m, "display_name", None) or m.name) for m in (model.measures or [])]
        plan = [{"type": "card", "measure": m} for m in measures[:4]]
        logger.info("PBIR: no AI plan — using %d default cards", len(plan))
    else:
        logger.info("PBIR: rendering %d AI-planned visuals", len(plan))

    visuals = _plan_to_pbir_visuals(plan, model)
    page_id = _pbir_id()
    d = f"{report_folder}/definition"

    # Grow the page to fit everything (slicers can sit below 720px)
    page_h = PBIR_PAGE_H
    for v in visuals:
        p = v.get("position", {})
        page_h = max(page_h, int(p.get("y", 0)) + int(p.get("height", 0)) + 24)

    files = {
        f"{d}/report.json": json.dumps(_pbir_report_json(), indent=2),
        f"{d}/version.json": json.dumps(_pbir_version_json(), indent=2),
        f"{d}/pages/pages.json": json.dumps(_pbir_pages_json([page_id], page_id), indent=2),
        f"{d}/pages/{page_id}/page.json": json.dumps(
            _pbir_page_json(page_id, "Overview", page_h), indent=2),
    }
    for vis in visuals:
        vid = vis["name"]
        files[f"{d}/pages/{page_id}/visuals/{vid}/visual.json"] = json.dumps(vis, indent=2)

    return files


def _plan_to_containers(plan: list, model: SemanticModel) -> list:
    """
    Render a neutral visual plan (from visual_planner) into legacy report.json
    visual containers, reusing the SAME verified config builders as the default
    layout. Placement uses a fixed, safe grid — the AI decides WHAT, we decide
    WHERE, which avoids overlap bugs.

    Page capacity (single page, scrolls if tall): up to
    PAGE_MAX_CARDS + PAGE_MAX_CHARTS + PAGE_MAX_SLICERS visuals. When we add
    multi-page support later, overflow past these caps becomes page 2, 3, ...
    """
    PAGE_MAX_CARDS   = 4
    PAGE_MAX_CHARTS  = 6      # 2 rows x 3
    PAGE_MAX_SLICERS = 5
    ...
    # Title
    containers.append({
        "config": _legacy_title_config(model.project_name or "Report"),
        "filters": "[]", "height": 50.0, "width": 1232.0,
        "x": 24.0, "y": 20.0, "z": 0.0,
    })

    # Split the plan by visual kind
    cards   = [v for v in plan if v.get("type") == "card"]
    charts  = [v for v in plan if v.get("type") in ("bar", "line", "donut")]
    slicers = [v for v in plan if v.get("type") == "slicer"]

    # KPI cards row (y=90), up to 4
    cards = cards[:PAGE_MAX_CARDS]
    if fact_table and cards:
        n = len(cards)
        margin = 24
        card_w = (1280 - 2 * margin - (n - 1) * margin) / n
        for i, v in enumerate(cards):
            cx = margin + i * (card_w + margin)
            cfg = _legacy_card_config(v["measure"], fact_table, cx, 90, card_w, 120, i + 1)
            containers.append({
                "config": cfg, "filters": "[]", "height": 120.0,
                "width": card_w, "x": cx, "y": 90.0, "z": 0.0})

    # Charts: up to 6 across TWO rows (3 per row) so busy dashboards fit.
    # Row 1 at y=240, row 2 at y=510. Each chart 400x250.
    _VT = {"bar": "clusteredBarChart", "line": "lineChart", "donut": "donutChart"}
    chart_h, chart_w, gap = 250, 400, 16
    col_x = [24, 24 + (chart_w + gap), 24 + 2 * (chart_w + gap)]   # 3 columns
    row_y = [240, 510]                                             # 2 rows
    placed = 0
    for v in charts[:PAGE_MAX_CHARTS]:
        if not fact_table:
            break
        vt = _VT[v["type"]]
        if v["type"] == "line":
            cat_table, cat_col = v.get("axis_table"), v.get("axis_column")
        else:
            cat_table, cat_col = v.get("category_table"), v.get("category_column")
        if not (cat_table and cat_col):
            continue
        cx = col_x[placed % 3]
        cy = row_y[placed // 3]
        cfg = _legacy_chart_config(vt, cat_col, cat_table, v["measure"], fact_table,
                                   cx, cy, chart_w, chart_h, 20 + placed, measure_is_real=True)
        containers.append({
            "config": cfg, "filters": "[]", "height": float(chart_h),
            "width": float(chart_w), "x": float(cx), "y": float(cy), "z": 0.0})
        placed += 1

    # Slicers: compact strip below the charts (y depends on how many chart rows
    # were used), up to 5 across.
    slicer_y = 510 + chart_h + 20 if placed > 3 else 240 + chart_h + 20
    sx, sw = 24, 240
    for j, v in enumerate(slicers[:PAGE_MAX_SLICERS]):
        cfg = _legacy_slicer_config(v["column"], v["table"], sx, slicer_y, sw, 150, 30 + j)
        containers.append({
            "config": cfg, "filters": "[]", "height": 150.0,
            "width": float(sw), "x": float(sx), "y": float(slicer_y), "z": 0.0})
        sx += sw + 16

    return containers


def build_pbir_report_files(model: SemanticModel, report_folder: str,
                            user_requests: str = "") -> dict:
    """
    AI-planned report generation with a safety fallback.

    Planner-first: ask the AI Visual Planner what visuals to build (honoring any
    user requests from the protocol/prompt), then render them. If the planner is
    unavailable or returns nothing usable, fall back to _build_report_default —
    the proven fixed layout — so a planner hiccup never yields a blank dashboard.
    """
    plan = []
    try:
        from .visual_planner import build_visual_plan
        plan = build_visual_plan(model, user_requests or "")
    except Exception as exc:      # ImportError, API failure, anything
        logger.warning("Visual planner unavailable (%s) — using default layout", exc)

    if not plan:
        logger.info("No AI plan — rendering default layout")
        return _build_report_default(model, report_folder)

    logger.info("Rendering %d AI-planned visuals", len(plan))
    visual_containers = _plan_to_containers(plan, model)
    page_id = _pbir_id()

    # Size the page to its content (default 720; grow if charts span two rows)
    max_bottom = 720.0
    for c in visual_containers:
        max_bottom = max(max_bottom, float(c["y"]) + float(c["height"]))
    page_height = max_bottom + 24.0
    ...
    section = {
        "config": "{}", "displayName": "Overview", "displayOption": 1,
        "filters": "[]", "height": page_height, "name": page_id,
        "visualContainers": visual_containers, "width": 1280.0,
    }
    report_json = {
        "config": report_config,
        "layoutOptimization": 0,
    ...
    return {f"{report_folder}/report.json": json.dumps(report_json, indent=2)}


def build_pbip_files(model: SemanticModel, bim_str: str, report_str: str,
                     user_requests: str = "") -> dict:
    """
    ...
    # Toggle: modern PBIR (definition/ folder) vs legacy (flat report.json).
    # Modern PBIR is required by Power BI Desktop 2026+. Set env
    # PBI_USE_LEGACY_REPORT=1 to fall back to the old flat format for testing.
    use_legacy = os.getenv("PBI_USE_LEGACY_REPORT", "0") == "1"
    ...
    if use_legacy:
        logger.info("Report format: LEGACY (flat report.json)")
        files.update(build_pbir_report_files(model, report_folder, user_requests))
    else:
        logger.info("Report format: MODERN PBIR (definition/ folder)")
        files.update(build_pbir_modern_files(model, report_folder, user_requests))
...
def export_powerbi_package(model: SemanticModel,
    ...
    pbip_files = build_pbip_files(model, bim_str, report_str, user_requests)
